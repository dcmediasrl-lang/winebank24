...
export default async function CantinaReportsPage() {
  ...
  if (!cantina) return <p className="text-stone-400">Profilo cantina non trovato.</p>;
  ...
  const typeColor: Record<string, string> = {
    ...
    FEE: "bg-stone-100 text-stone-600",
  };

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-stone-900">Reportistica</h1>

      {/* KPI */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label="Fatturato totale" value={`€ ${totals.fatturato.toFixed(2)}`} icon={Euro} color="text-green-600" />
        <KpiCard label="Royalties incassate" value={`€ ${totals.royalties.toFixed(2)}`} icon={TrendingUp} color="text-amber-600" />
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            {[
              ...
              { key: "SOLD", label: "Venduti", color: "bg-blue-100 text-blue-700" },
              { key: "BURN_REQUESTED", label: "Bottiglia richiesta", color: "bg-orange-100 text-orange-700" },
              { key: "BURNED", label: "Bruciati", color: "bg-red-100 text-red-700" },
            ].map(({ key, label, color }) => (
      ...
      <Card>
        ...
        <CardContent>
          {collections.length === 0 ? (
            <p className="text-stone-400 text-sm">Nessuna collezione ancora.</p>
          ) : (
                ...
                return (
                  <div key={col.id} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-stone-500">{col.minted} / {col.totalSupply} mintati ({pct}%)</span>
                    </div>
      ...
      <Card>
        ...
        <CardContent>
          {transactions.length === 0 ? (
            <p className="text-stone-400 text-sm text-center py-6">Nessuna transazione ancora.</p>
          ) : (
